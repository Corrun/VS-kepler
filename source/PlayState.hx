...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'kepler' | 'trappist' | 'wasp-107b':
			...
			case 'space': 
				{
					...
					var bg:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/background.', 'kepler'));
					bg.setGraphicSize(Std.int(bg.width * 2.7));
					bg.setGraphicSize(Std.int(bg.height * 2.7));
					bg.antialiasing = true;
					bg.scrollFactor.set(1, 1);
					bg.active = false;
					add(bg);

					var Stars:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/Stars', 'kepler'));
					Stars.setGraphicSize(Std.int(Stars.width * 2.7));
					Stars.setGraphicSize(Std.int(Stars.height * 2.7));
					Stars.antialiasing = true;
					Stars.scrollFactor.set(1, 1);
					Stars.active = false;
					add(Stars);

					var Planet2:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/Planet_2', 'kepler'));
					Planet2.setGraphicSize(Std.int(Planet2.width * 2.7));
					Planet2.setGraphicSize(Std.int(Planet2.height * 2.7));
					Planet2.antialiasing = true;
					Planet2.scrollFactor.set(1, 1);
					Planet2.active = false;
					add(Planet2);
					
					var Planet:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/Planet_1', 'kepler'));
					Planet.setGraphicSize(Std.int(Planet.width * 2.7));
					Planet.setGraphicSize(Std.int(Planet.height * 2.7));
					Planet.antialiasing = true;
					Planet.scrollFactor.set(1, 1);
					Planet.active = false;
					add(Planet);

					var Saturn:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/Saturn', 'kepler'));
					Saturn.setGraphicSize(Std.int(Saturn.width * 2.7));
					Saturn.setGraphicSize(Std.int(Saturn.height * 2.7));
					Saturn.antialiasing = true;
					Saturn.scrollFactor.set(1, 1);
					Saturn.active = false;
					add(Saturn);

					var Rocket:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/Rocket', 'kepler'));
					Rocket.setGraphicSize(Std.int(Rocket.width * 2.7));
					Rocket.setGraphicSize(Std.int(Rocket.height * 2.7));
					Rocket.antialiasing = true;
					Rocket.scrollFactor.set(1, 1);
					Rocket.active = false;
					add(Rocket);
					
					var Rocks:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/Rocks', 'kepler'));
					Rocks.setGraphicSize(Std.int(Rocks.width * 2.7));
					Rocks.setGraphicSize(Std.int(Rocks.height * 2.7));
					Rocks.antialiasing = true;
					Rocks.scrollFactor.set(1, 1);
					Rocks.active = false;
					add(Rocks);

					var floor:FlxSprite = new FlxSprite(-100, -100).loadGraphic(Paths.image('space/floor1', 'kepler'));
					floor.setGraphicSize(Std.int(floor.width * 2.7));
					floor.setGraphicSize(Std.int(floor.height * 2.7));
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'kepler':
				camPos.set(dad.getGraphicMidpoint().x + 400, dad.getGraphicMidpoint().y + 400);
				dad.x -= 190;
				dad.y -= 200;
			case 'kepler-stage-3':
				dad.x -= 190;
	...
	function keplerTrans(?dialogueBox:DialogueBox):Void
	{
		dad.visible = false;

		var cutdad:FlxSprite = new FlxSprite(dad.x,dad.y);
		cutdad.frames = Paths.getSparrowAtlas('trans/Kepler_Transformation_Assets', 'kepler');
		cutdad.animation.addByPrefix('trans', 'Kepler trans', 8);
		cutdad.updateHitbox();
		add(cutdad);

		var FrontEnd = new FlxSprite().makeGraphic(FlxG.width + 200, FlxG.height + 200, 0xFF84E0FF);
		FrontEnd.alpha = 0;
		add(FrontEnd);

		//blocking key pressing during cutscene
		inCutscene = true;

		cutdad.animation.play('trans');

		//Hiding the HUD and changing the camera position instantly
		camHUD.visible = true;
		camFollow.x = dad.frameWidth - 200;
		camFollow.y = dad.frameHeight + 180;
		FlxG.camera.zoom = 0.9;


		new FlxTimer().start(4, function(swagTimer:FlxTimer)
		{
			FrontEnd.x = dad.frameWidth - 1000;
			FrontEnd.y = dad.frameHeight -500;
			FlxTween.tween(FrontEnd,{alpha: FrontEnd.alpha + 1}, 1.4, {ease: FlxEase.expoInOut});
		});

		new FlxTimer().start(6, function(swagTimer:FlxTimer) {
			cutdad.animation.pause();
		});

		new FlxTimer().start(7, function(swagTimer:FlxTimer)
		{
			cutdad.animation.resume();
			remove(FrontEnd);
		});

		//when the 5 seconds of this timer ends the black screen and animation sprites are deleted,
		//the HUD turns visible again, the camera tweens into the normal zoom and the song starts
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		|| StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'kepler'
		|| StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'trappist'
		|| StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'wasp-107b')
		{
			remove(black);
	...
	*/
	...
	override function beatHit()
	{
		...
		if (dad.curCharacter == 'kepler' && curSong == 'trappist' && curBeat % 20 == 15)
			{
				dad.playAnim('twitch', true);
			}
